...
var resizePizzas = function(size) {
	...
	// PO: Since this method now returns the size of the image in %, renaming it.
	// function determineDx (elem, size) {
	function determineWidth (size) {
		// PO: Letting the browser adjust the dimensions of elements based on screen and window size is preferrred.
		// PO: Hence instead of calculating the absolute size (i.e. width) of pizza image based on size toggle,
		// PO: we specify the width in % while ensuring that it creates images of the same size as the absolute version.
		...
		// PO: Not required as we specify width in %
		// var dx = (newSize - oldSize) * windowWidth;

		// PO: Send the new size in percent
		// return dx;
		return newSize;
	}
	...
	function changePizzaSizes(size) {
		// PO: As the number of pizza containers won't change in the lifetime of the 'for' loop, we fetch and store
		// PO: before the loop commences and used it instead
		...
		for (var i = 0; i < randomPizzaContainers.length; i++) {
			// PO: Rather than computing the absolute width of pizza images, we specify the attribute in % and 
			// PO: let the browser do the required calculations
...
document.addEventListener('DOMContentLoaded', function() {
	...
	for (var i = 0; i < 200; i++) {
		...
		// PO: Moved to CSS as they apply to all images
		...
		// PO: Rather than running another 'for' loop in updatePositions() to update the 'left' style attribute,
		// PO: we add it here safe in the knowledge that value of 'scrollTop' will be 0. Tests showed that this 
		// PO: approach shaved approximately 20ms from the 'ParseHTML' step after the DOM is loaded.
	...
	// PO: Work of this call performed while creating the element
	// updatePositions();
